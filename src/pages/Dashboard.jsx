    ...
    function Dashboard() {
      ...
      const handleParameterChange = (e, paramKey) => {
        ...
        if (logicLimitOption === "option1") {
          ...
          getLimit.Limit_Listrik = paramKey === 'listrik' ? value : getLimit.Limit_Listrik;
          getLimit.User = userGlobal.name
          getLimit.Created_date = createdDate
          getLimit.Created_time = createdTime
        }
...
// ==============================================================   SUBMIT   ============================================================================================  
    ...
    const createJamPost = async () => {
      let tempData = mainObject;
      let response = await axios.post("http://10.126.15.137:8002/part/CreateJam", tempData);
      console.log(tempData);
    };
    ...
    const createParameterPost = async () => {
      ...
      let response2 = await axios.post("http://10.126.15.137:8002/part/CreateParameter", newParameterData);
    };
    ...
    const createLimit = async () => {
      ...
      try {
        let response = await axios.post("http://10.126.15.137:8002/part/CreateLimit", limitData);
                ...
                <ModalBody>
                    ...
                    <Box mb={4}>
                      ...
                      <SimpleGrid columns={{ base: 1, md: 3 }} spacing={4}>
                        {parameterConfigs
                          .filter(({ key }) => {
                            if (logicLimitOption === "option1") return key === 'water';
                            if (logicLimitOption === "option2") return key === 'gas';
                            if (logicLimitOption === "option3") return key === 'listrik';
                            return false;
                          })
                          .map(({ key, label, unit }) => (
                  ...
                  <svg aria-hidden="true" fill="none" viewBox="0 0 24 24" stroke="currentColor" className="h-6 w-6">
                    <path strokeLinecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                      ...
                      <div>
                        ...
                        <span
                              className="flex items-center gap-1 text-[16px] pt-1 justify-end font-medium text-gray-500">{((data.MVMDP/getLimit.Limit_Listrik )*100).toFixed(1)}%
                              <svg
                                className="fill-gray-500"
                                width="10"
                                height="11"
